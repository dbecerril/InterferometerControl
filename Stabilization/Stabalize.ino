 ...
 * This version is based on trial7, it is my first attempt to comunicate
...
int     delayTime = 100, npid = 510, counter1 = 0,Nstability = 1000;
bool    feedback_on = false, preScanFase  = false;
int     stabilityCount = 0,N_AvgWindow  = 30;
...
// for 780nm 
//float kp = 1.0 , ki = 0.7*kp;

// for 633 nm  160mV drive piezo
float kp = 1.0 , ki =  1.0*kp  ;
//float kp = 0.4 , ki = 0.4;
double aggKp = kp, aggKi  = ki,  aggKd  = 0.0;
double consKp= 0.4*kp, consKi = 0.4*ki, consKd = 0.0;
...
void choosePidParameters(float check){
  ...
  else if ( 0.2*pi < abs(check) < 0.6*pi)  
  {
     //we're far from setpoint, use aggressive tuning parameters
    myPID.SetTunings(0.7*consKp, 0.7*consKi, consKd);

  }
  else if ( 0 < abs(check) < 0.2*pi ) 
    myPID.SetTunings(0.5*consKp, 0.5*consKi, consKd);
...
void loop()
{
  ...
  choosePidParameters(checkTheta);
  ...
  if (!feedback_on  ){
    ...
    thetaNow = 0;
    for (int i = 1;  i < N_AvgWindow + 1; i++ ){
      V0      = analogRead(inPin0)-vtd*2.5;
      V1      = analogRead(inPin1)-vtd*2.5;
      thetaNow += atan2(dtv*V0,dtv*V1) + (2*nn+1  )*pi;
      delay(5);
  }
    thetaNow = thetaNow / N_AvgWindow;
  ...
  if(counter1 > 400){
    sendToComputer(millis(),checkTheta,dtp*VdOutNow);
